...
import { getBots, createTask, getBots as getBotsAll } from '../api/client';
...
export default function Bots() {
  ...
  const [filters, setFilters] = useState({ country: '', tag: '', search: '', status: '' });
  ...
  const loadBots = async () => {
    ...
    try {
      ...
      setBots(data);
      // Se a API não retornar total, calculamos de forma simplificada.
      // Idealmente a API deve retornar o total de registros para paginação correta.
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        <input
          type="text"
          name="search"
          placeholder="Buscar por bot_id, tag ou número"
        ...
        <button
          onClick={() => setFilters({ country: '', tag: '', search: '', status: '' })}
          className="px-3 py-2 border rounded-md hover:bg-gray-200 dark:hover:bg-gray-700"
        >
          Limpar
        </button>
      </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {bots.map(bot => {
                  const online = isOnline(bot.last_seen);
                  return (
                    <tr key={bot.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 text-xs rounded-full ${online ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
